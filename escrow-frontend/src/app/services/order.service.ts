...
export class OrderService {
  ...
  nextPage(link:number,cmp:string,query:string){
    if(cmp == 'seller-orders'){
      ...
      return this.http.get<any>(`${environment.api}/orders?limit=${environment.paginationLimit}&offset=${(link)}`+query) 
    }
  }
  ...
  storeTransactionResponse(reference:string,transaction:any,order_public_id:string,success:boolean){
    return this.http.post<any>(`${environment.api}/transactions?status=${success}`,{reference:reference,transaction:transaction,order_public_id:order_public_id})
  }
}
