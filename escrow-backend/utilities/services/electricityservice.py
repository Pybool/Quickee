...
class ElectricityPurchaseService(object):
    
    def electricityRoutine(self, data, cart_reference='',order_id=''):
            
            try:
                order_id = order_id
                cart_reference = cart_reference
                # Select the Api provider endpoint to use ###########################
                vtung = lambda : VtungRequestParser(ops,username, api_password, data,"electricity", cart_reference=cart_reference,order_id=order_id)
                ...
                print("An error ocurred in electricity purchase ",e)